...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('binea-upms-systemid', systemid);
		$.cookie('binea-upms-systemname', systemname);
		$.cookie('binea-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('binea-upms-systemid') || 1;
	var systemname = $.cookie('binea-upms-systemname') || 'binea-upms-server';
	var systemtitle = $.cookie('binea-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
